...
import { Search, LayoutGrid, List as ListIcon, SlidersHorizontal, Printer } from 'lucide-react';
...
export default function Printers() {
  ...
  const filteredProducts = useMemo(() => {
    return MOCK_PRODUCTS.filter(p => {
      const isPrinter = p.category === ProductCategory.PRINTING;
    ...
    <div className="min-h-screen bg-slate-50 pb-20 pt-8 text-slate-900">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex items-center gap-2 text-xs font-medium text-slate-400 uppercase tracking-widest">
          ...
          <span>Catalog</span>
          <span>/</span>
          <span className="text-brand-secondary">Printers</span>
        </div>

        <div className="mb-12">
          <h1 className="font-display text-4xl font-bold text-slate-900 sm:text-6xl">
            OFFICE <span className="text-brand-secondary italic">PRINTERS</span>
          </h1>
          <p className="mt-4 max-w-2xl text-slate-500 font-medium">
            High-speed precision for the modern workspace. From efficient desktop lasers to high-volume 
            enterprise multi-function systems, manage your documentation workflow with reliability.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-4">
          ...
          <aside className="hidden space-y-8 lg:block">
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <h3 className="mb-4 font-display text-xs font-bold uppercase tracking-widest text-slate-400">Print Tech</h3>
              <div className="space-y-3">
                {['Laser Printers', 'Inkjet Printers', 'Multi-function MFP', 'Label Printers', 'Wide Format', 'Scanners'].map(cat => (
            ...
            <div className="rounded-2xl border border-brand-secondary/20 bg-slate-900 p-6 shadow-xl shadow-slate-200">
              ...
              <h4 className="font-display text-lg font-bold text-white uppercase italic tracking-tighter">Managed Print</h4>
              <p className="mt-2 text-xs leading-relaxed text-slate-400">
                Lower your cost per page by up to 30% with our managed print services. Automated toner replenishment and proactive maintenance.
              </p>
          ...
          <div className={cn(
            ...
            {filteredProducts.length === 0 && (
              <div className="col-span-full py-20 text-center">
                ...
                <p className="mt-4 font-display text-xl font-bold">No printers found</p>
                <p className="text-sm text-slate-400 font-medium">Try searching for laser, inkjet, or specific brands.</p>
              </div>
            )}
          </div>
        </div>
