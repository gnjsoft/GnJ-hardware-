...
import { Search, LayoutGrid, List as ListIcon, SlidersHorizontal, Zap } from 'lucide-react';
...
export default function Networking() {
  ...
  const filteredProducts = useMemo(() => {
    return MOCK_PRODUCTS.filter(p => {
      const isNetworking = p.category === ProductCategory.NETWORKING;
      const matchesSearch = p.name.toLowerCase().includes(searchQuery.toLowerCase()) || 
                            p.brand.toLowerCase().includes(searchQuery.toLowerCase()) ||
                            p.sku.toLowerCase().includes(searchQuery.toLowerCase());
      return isNetworking && matchesSearch;
    });
  }, [searchQuery]);
    ...
    <div className="min-h-screen bg-slate-50 pb-20 pt-8 text-slate-900">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex items-center gap-2 text-xs font-medium text-slate-400 uppercase tracking-widest">
          ...
          <span>Catalog</span>
          <span>/</span>
          <span className="text-brand-secondary">Networking</span>
        </div>

        <div className="mb-12">
          <h1 className="font-display text-4xl font-bold text-slate-900 sm:text-6xl">
            ENTERPRISE <span className="text-brand-secondary italic">NETWORKING</span>
          </h1>
          <p className="mt-4 max-w-2xl text-slate-500 font-medium">
            Building the foundation of the digital world. High-speed switches, reliable wireless access points, 
            and intelligent security appliances for seamless connectivity.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-4">
          ...
          <aside className="hidden space-y-8 lg:block">
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <h3 className="mb-4 font-display text-xs font-bold uppercase tracking-widest text-slate-400">Network Type</h3>
              <div className="space-y-3">
                {['Core Switches', 'Access Switches', 'Routers', 'Access Points', 'Firewalls', 'SFP Transceivers'].map(cat => (
            ...
            <div className="rounded-2xl border border-brand-secondary/20 bg-indigo-900 p-6 shadow-xl shadow-indigo-100">
              ...
              <h4 className="font-display text-lg font-bold text-white uppercase italic tracking-tighter">SD-WAN Ready</h4>
              <p className="mt-2 text-xs leading-relaxed text-indigo-200/70">
                Optimize your branch connectivity with automated path selection and centralized management policies.
              </p>
          ...
          <div className={cn(
            ...
            {filteredProducts.length === 0 && (
              <div className="col-span-full py-20 text-center">
                ...
                <p className="mt-4 font-display text-xl font-bold">No networking gear found</p>
                <p className="text-sm text-slate-400 font-medium">Try searching for switches, routers, or specific model numbers.</p>
              </div>
            )}
          </div>
        </div>
