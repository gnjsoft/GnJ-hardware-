...
export default function AccessControl() {
  ...
  const filteredProducts = useMemo(() => {
    return MOCK_PRODUCTS.filter(p => {
      const isAccessControl = p.category === ProductCategory.ACCESS_CONTROL;
      const matchesSearch = p.name.toLowerCase().includes(searchQuery.toLowerCase()) || 
                            p.brand.toLowerCase().includes(searchQuery.toLowerCase()) ||
                            p.sku.toLowerCase().includes(searchQuery.toLowerCase());
      return isAccessControl && matchesSearch;
    });
  }, [searchQuery]);
    ...
    <div className="min-h-screen bg-slate-50 pb-20 pt-8 text-slate-900">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex items-center gap-2 text-xs font-medium text-slate-400 uppercase tracking-widest">
          ...
          <span>Catalog</span>
          <span>/</span>
          <span className="text-brand-secondary">Access Control</span>
        </div>

        <div className="mb-12">
          <h1 className="font-display text-4xl font-bold text-slate-900 sm:text-6xl">
            PRECISION <span className="text-brand-secondary italic">ACCESS CONTROL</span>
          </h1>
          <p className="mt-4 max-w-2xl text-slate-500 font-medium">
            Define who goes where, and when. From biometric scanners to cloud-managed door controllers, 
            secure your facility with industry-leading identity verification systems.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-4">
          ...
          <aside className="hidden space-y-8 lg:block">
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <h3 className="mb-4 font-display text-xs font-bold uppercase tracking-widest text-slate-400">Technology</h3>
              <div className="space-y-3">
                {['Biometrics', 'Facial Recognition', 'RFID/Proximity', 'Mobile Keys', 'Door Controllers', 'Electric Strikes'].map(cat => (
            ...
            <div className="rounded-2xl border border-brand-secondary/20 bg-slate-900 p-6 shadow-xl shadow-slate-200">
              ...
              <h4 className="font-display text-lg font-bold text-white uppercase italic tracking-tighter">Zero Trust Access</h4>
              <p className="mt-2 text-xs leading-relaxed text-slate-400">
                Implement multi-factor authentication for physical spaces. Integrate with Active Directory for unified identity management.
              </p>
          ...
          <div className={cn(
            ...
            {filteredProducts.length === 0 && (
              <div className="col-span-full py-20 text-center">
                ...
                <p className="mt-4 font-display text-xl font-bold">No access control gear found</p>
                <p className="text-sm text-slate-400 font-medium">Try searching for readers, cards, or specific biometrics.</p>
              </div>
            )}
          </div>
        </div>
