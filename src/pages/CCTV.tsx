...
import { Search, LayoutGrid, List as ListIcon, SlidersHorizontal, Camera } from 'lucide-react';
...
export default function CCTV() {
  ...
  const filteredProducts = useMemo(() => {
    return MOCK_PRODUCTS.filter(p => {
      const isCCTV = p.category === ProductCategory.SURVEILLANCE;
      const matchesSearch = p.name.toLowerCase().includes(searchQuery.toLowerCase()) || 
                            p.brand.toLowerCase().includes(searchQuery.toLowerCase()) ||
                            p.sku.toLowerCase().includes(searchQuery.toLowerCase());
      return isCCTV && matchesSearch;
    });
  }, [searchQuery]);
    ...
    <div className="min-h-screen bg-slate-50 pb-20 pt-8 text-slate-900">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex items-center gap-2 text-xs font-medium text-slate-400 uppercase tracking-widest">
          ...
          <span>Catalog</span>
          <span>/</span>
          <span className="text-brand-secondary">CCTV</span>
        </div>

        <div className="mb-12">
          <h1 className="font-display text-4xl font-bold text-slate-900 sm:text-6xl">
            SECURITY <span className="text-brand-secondary italic">SURVEILLANCE</span>
          </h1>
          <p className="mt-4 max-w-2xl text-slate-500 font-medium">
            Advanced visual intelligence for total situational awareness. From AI-enabled dome cameras to high-capacity 
            network video recorders, we provide the eyes for your enterprise.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-4">
          ...
          <aside className="hidden space-y-8 lg:block">
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <h3 className="mb-4 font-display text-xs font-bold uppercase tracking-widest text-slate-400">Unit Type</h3>
              <div className="space-y-3">
                {['IP Cameras', 'Dome Cameras', 'PTZ Cameras', 'NVR/DVR Systems', 'Thermal Imaging', 'Body Cameras'].map(cat => (
            ...
            <div className="rounded-2xl border border-brand-secondary/20 bg-slate-900 p-6 shadow-xl shadow-slate-200">
              ...
              <h4 className="font-display text-lg font-bold text-white uppercase italic tracking-tighter">AI Vision</h4>
              <p className="mt-2 text-xs leading-relaxed text-slate-400">
                Reduce false alarms by 90% with our new deep-learning analytics that distinguish between humans, animals, and environmental motion.
              </p>
          ...
          <div className={cn(
            ...
            {filteredProducts.length === 0 && (
              <div className="col-span-full py-20 text-center">
                ...
                <p className="mt-4 font-display text-xl font-bold">No surveillance found</p>
                <p className="text-sm text-slate-400 font-medium">Try broadening your search or specific categories like 'NVR' or 'PTZ'.</p>
              </div>
            )}
          </div>
        </div>
