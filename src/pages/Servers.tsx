...
import { Search, LayoutGrid, List as ListIcon, SlidersHorizontal, Server } from 'lucide-react';
...
export default function Servers() {
  ...
  const filteredProducts = useMemo(() => {
    return MOCK_PRODUCTS.filter(p => {
      const isServer = p.category === ProductCategory.SERVERS;
      const matchesSearch = p.name.toLowerCase().includes(searchQuery.toLowerCase()) || 
                            p.brand.toLowerCase().includes(searchQuery.toLowerCase()) ||
                            p.sku.toLowerCase().includes(searchQuery.toLowerCase());
      return isServer && matchesSearch;
    });
  }, [searchQuery]);
    ...
    <div className="min-h-screen bg-slate-50 pb-20 pt-8 text-slate-900">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex items-center gap-2 text-xs font-medium text-slate-400 uppercase tracking-widest">
          ...
          <span>Catalog</span>
          <span>/</span>
          <span className="text-brand-secondary">Servers</span>
        </div>

        <div className="mb-12">
          <h1 className="font-display text-4xl font-bold text-slate-900 sm:text-6xl">
            DATA CENTER <span className="text-brand-secondary italic">SERVERS</span>
          </h1>
          <p className="mt-4 max-w-2xl text-slate-500 font-medium">
            Powering the modern enterprise. Scale your infrastructure with high-density compute, 
            resilient storage nodes, and AI-optimized acceleration platforms.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-4">
          ...
          <aside className="hidden space-y-8 lg:block">
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <h3 className="mb-4 font-display text-xs font-bold uppercase tracking-widest text-slate-400">Form Factor</h3>
              <div className="space-y-3">
                {['1U Rackmount', '2U Rackmount', 'Mini Towers', 'Blade Chassis', 'Edge Computing'].map(cat => (
            ...
            <div className="rounded-2xl border border-brand-secondary/20 bg-slate-900 p-6 shadow-xl shadow-slate-200">
              ...
              <h4 className="font-display text-lg font-bold text-white uppercase italic tracking-tighter">Liquid Cooling</h4>
              <p className="mt-2 text-xs leading-relaxed text-slate-400">
                Facing heat density challenges? Our new DLC (Direct Liquid Cooling) systems reduce TCO and support TDP up to 500W.
              </p>
          ...
          <div className={cn(
            ...
            {filteredProducts.length === 0 && (
              <div className="col-span-full py-20 text-center">
                ...
                <p className="mt-4 font-display text-xl font-bold">No servers match your criteria</p>
                <p className="text-sm text-slate-400 font-medium">Try our unified search or contact support for custom builds.</p>
              </div>
            )}
          </div>
        </div>
